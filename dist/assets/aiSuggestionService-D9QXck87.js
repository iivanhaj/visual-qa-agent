...
Be concise and practical.`,r=(await s.chat.completions.create({model:"gpt-5-mini-2025-08-07",messages:[{role:"system",content:"You are an expert web developer helping to fix bugs. Provide clear, actionable advice."},{role:"user",content:o}],max_completion_tokens:300,temperature:.7})).choices[0].message.content||"";return{...e,suggestion:r,resources:[...e.resources||[],"✨ Enhanced with AI suggestions"]}}catch(s){return console.error("AI enhancement failed:",s),e}}async generateExecutiveSummary(e){if(!this.apiKey)return this.getBasicSummary(e);try{const a=new u({apiKey:this.apiKey,dangerouslyAllowBrowser:!0}),s=e.allIssues.filter(n=>n.severity==="critical"),o=e.allIssues.filter(n=>n.severity==="high"),c=`Analyze this webpage quality report and provide an executive summary:
...
Be professional and concise.`;return(await a.chat.completions.create({model:"gpt-5-mini-2025-08-07",messages:[{role:"system",content:"You are a web quality consultant providing executive summaries."},{role:"user",content:c}],max_completion_tokens:400,temperature:.7})).choices[0].message.content||this.getBasicSummary(e)}catch(a){return console.error("AI summary generation failed:",a),this.getBasicSummary(e)}}async enhanceCriticalIssues(e,a){if(!this.apiKey||e.length===0)return e;const s=e.filter(o=>o.severity==="critical"||o.severity==="high");if(s.length===0)return e;try{const o=new u({apiKey:this.apiKey,dangerouslyAllowBrowser:!0}),c=s.slice(0,5),r=c.map((i,l)=>`Issue ${l+1}: ${i.title}
Type: ${i.type}
Severity: ${i.severity}
...
...`,h=((await o.chat.completions.create({model:"gpt-5-mini-2025-08-07",messages:[{role:"system",content:"You are an expert web developer. Provide concise, actionable fixes."},{role:"user",content:n}],max_completion_tokens:500,temperature:.7})).choices[0].message.content||"").split(/Issue \d+:/g).slice(1),g=c.map((i,l)=>{var m;return{...i,suggestion:((m=h[l])==null?void 0:m.trim())||i.suggestion,resources:[...i.resources||[],"✨ AI-Enhanced Fix"]}}),p=new Map(g.map(i=>[i.id,i]));return e.map(i=>p.get(i.id)||i)}catch(o){return console.error("Batch AI enhancement failed:",o),e}}getBasicSummary(e){const a=e.healthScore.overall;let s="";return a>=80?s="Excellent! Your page is in great shape with minimal issues.":a>=60?s="Good foundation, but several areas need attention.":a>=40?s="Significant issues detected. Prioritize critical and high severity fixes.":s="Critical attention required. Multiple serious issues affecting quality.",`${s}
